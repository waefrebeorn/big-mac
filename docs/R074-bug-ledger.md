...
1. comp_pull allocates fixed 4096x4096 RoD regardless of graph format
2. wb_node_pull clips ROI against hardcoded 4096x4096, not node format
3. wb_node_pull_request same hardcoded 4096 clip
4. No graph-level resolution concept — every node invents its own w/h
5. src_color ignores ROI: writes ROI but leaves rest zeroed (black frame if ROI partial)
6. src_text ignores ROI entirely; draws full frame always
7. trans_pull iterates a->w*a->h linearly, ignoring rx/ry/rw/rh
8. trans_pull has NO a-vs-b size mismatch guard (OOB read on nested graphs)
9. trans out->roi set from request even though loop covers full frame (lie)
10. No resampler/scaler node exists — mismatched sizes can't be reconciled
11. win_cx/cy normalized coords assume square pixels (aspect distortion on anamorphic)
12. vignette maxd uses corner distance; Vegas uses width/2 (different falloff shape)
13. text x/y/scale are absolute pixels — break when frame size changes
14. composite output alpha starts 0; black bg shows where nothing drawn (no background plate option)
15. PPM writer quantizes with +0.5 round but no clamp >1.0 values (wraps to garbage)
16. PPM writer no gamma/color-tag; players assume sRGB — washed output
17. mp4 export hardcodes 64x64-friendly sizes only via caller; no aspect check
18. blur reads/writes full W,H buffers but only clears ROI of tmp (stale data at edges)
19. iris transition maxd assumes center origin; no off-center iris param
20. camera-shake/CGI overlay nodes not audited for DPI/coords consistency

## B. Color science
21. All blending done in sRGB space, not linear light (dark blends wrong)
22. glow adds excess*color without clamping (values >1 wrap in PPM)
23. primary grade applies gain BEFORE gamma (order: lift,gain,gamma — should be lift,gamma,gain per ASC)
24. white balance temp multiplies after grade ops (WB should be first)
25. HSL hue_shift parsed but unimplemented ((void)hsh) — parameter is dead
26. HSL selection softness linear, no smoothstep (banding at edges)
...
29. luma key uses Rec.709 weights on sRGB values (should be linear-light luma)
30. key_tol default falls back to `gain` param — semantic collision with exposure gain
31. sat computed around Rec.709 luma of sRGB values (same linear-light issue)
32. shadow pass of text draws rgba(0,0,0,0.6) by OVERWRITING pixel (not blend) — kills underlying video
33. text glyph write replaces RGBA instead of source-over blending with existing frame content
34. noise dissolve hash uses px_i*const ^ py_i*const — visible axis-aligned correlation pattern
35. split-flap jitter reuses same hash constants — correlated with checkerboard pattern
...
## D. Correctness / logic
50. D78 debug fprintf(stderr) left inside hot HSL pixel loop (spams + slows)
51. gain param override: `kv != 0 || e->gain == 0` — cannot set gain to exactly 0 when static nonzero
52. same "gain" param doubles as blur radius fallback — semantic overload
53. trans_pull `if (!a) return b;` leaks a's siblings when one input missing mid-graph
54. transition duration relative to t=0 only — no start-time offset param (can't place at t=5s)
55. nested transitions receive outer t directly — inner transition sees full timeline, not local time
56. split-flap cw/ch integer div rounds down — last row/col cells wider than others
57. checkerboard cell size fixed 16px — not proportional to resolution
58. venetian strips fixed 16px — same issue
...
64. map dissolve threshold uses strict > (no soft knee) unlike documented feather
...
68. text cx param overrides x but y has no cy equivalent (asymmetric API)
...
73. no wb_node_set_start_time — all timing baked at t=0 origin
74. wb_compositor_export_mp4 uses system() — no error capture, shell injection via path possible
...
77. export_mp4 mkdir race between concurrent exports (pid-suffixed but rmdir non-empty fails silently)
...
88. wb_render --transition-frames hardcodes 2s duration and 64x64
89. wb_render video mode ignores --lufs (audio/video flags disjoint)
90. no version string in CLI outputs (can't tell which build rendered a file)
...
93. soundtrack is bare sine — no envelope, clicks at boundaries
94. scene colors are flat fields — no gradients/patterns to show grading
95. no fade-from-black at demo start / fade-to-black at end
...
98. showcase.mp4 has no faststart flag (moov at end — stalls web playback)
99. no poster/thumbnail extraction alongside exports
100. demo graph built ad-hoc in test file — no reusable builder API for apps
