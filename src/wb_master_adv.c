/* wb_master_adv.c — advanced mastering suite (iZotope Ozone style).
 *
...
void *wb_master_adv_create(uint32_t sr) {
    wb_master_adv *m = (wb_master_adv *)calloc(1, sizeof(*m));
    if (!m) return NULL;
    m->sr = sr;
    ...
    lr4_split_init(&m->bm_split, (float)sr, 120.0f);
...
void wb_master_adv_destroy(void *inst) {
    wb_master_adv *m = (wb_master_adv *)inst;
    if (!m) return;
    for (int b = 0; b < NBANDS; b++) {
...
void wb_master_adv_set_input_gain(void *inst, float db) {
    ((wb_master_adv *)inst)->input_gain_db = db;
}

void wb_master_adv_set_output_gain(void *inst, float db) {
    ((wb_master_adv *)inst)->output_gain_db = db;
}

void wb_master_adv_set_loudness_target(void *inst, float lufs) {
    ((wb_master_adv *)inst)->loudness_target_lufs = lufs;
}

void wb_master_adv_set_stereo_width(void *inst, float width) {
    wb_master_adv *m = (wb_master_adv *)inst;
...
void wb_master_adv_set_bass_mono(void *inst, int enable) {
    ((wb_master_adv *)inst)->bass_mono_enabled = enable ? 1 : 0;
}

/* Get current integrated LUFS estimate. */
float wb_master_adv_get_loudness(const void *inst) {
    const wb_master_adv *m = (const wb_master_adv *)inst;
    if (!m) return 0.0f;
...
float wb_master_adv_get_peak(const void *inst) {
    const wb_master_adv *m = (const wb_master_adv *)inst;
    if (!m) return 0.0f;
...
int wb_master_adv_process(void *inst, wb_sample *out_l, wb_sample *out_r,
                          const wb_sample *in_l, const wb_sample *in_r,
                          uint32_t frames) {
    wb_master_adv *m = (wb_master_adv *)inst;
    if (!m || !out_l || !out_r || !in_l || !in_r || frames == 0) return -1;
    ...
    for (uint32_t i = 0; i < frames; i++) {
        out_l[i] = in_l[i] * input_gain;
        out_r[i] = in_r[i] * input_gain;
    }
    ...
    for (uint32_t i = 0; i < frames; i++) {
        float in_l_s = out_l[i];
        float in_r_s = out_r[i];

        /* First split at 100 Hz */
        float lo_l = lr4_split_low(&m->split1, in_l_s);
        float lo_r = lr4_split_low(&m->split1, in_r_s);
        float hi_l = lr4_split_high(&m->split1, in_l_s);
        float hi_r = lr4_split_high(&m->split1, in_r_s);
    ...
    /* ---- Stage 4: Bass mono (fold <120 Hz to mono) ---- */
    /* Use a dedicated 120 Hz LR4 crossover to extract the bass portion,
     * then fold it to mono and blend back with the non-bass content. */
    if (m->bass_mono_enabled) {
        for (uint32_t i = 0; i < frames; i++) {
            /* Extract bass below 120 Hz from both channels */
            float bass_l = lr4_split_low(&m->bm_split, out_l[i]);
            float bass_r = lr4_split_low(&m->bm_split, out_r[i]);
            /* Non-bass content */
            float nonbass_l = out_l[i] - bass_l;
            float nonbass_r = out_r[i] - bass_r;
            /* Fold bass to mono */
            float mono_bass = (bass_l + bass_r) * 0.5f;
            /* Recombine */
            out_l[i] = nonbass_l + mono_bass;
            out_r[i] = nonbass_r + mono_bass;
        }
    }
